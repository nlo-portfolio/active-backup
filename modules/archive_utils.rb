...
module ArchiveUtils
  ...
  #
  # Parameters:
  #   paths (list): contains the file directory paths to archive.
  #
  # Returns: StringIO
  #
  def self.archive(paths)
    ...
    Gem::Package::TarWriter.new(tarfile) do |tar|
      Dir[File.join(paths, "**/*")].each do |file|
  ...
  def self.compress tarfile
    ...
    z.close  # writes the gzip footer.
  ...
  #
  # Parameters:
  #   task (BackupTask object): task containing current paths to backup.
  ...
  def self.encrypt password, data
    ...
    output = StringIO.new
    while chunk = data.read(16 * 1024) do
  ...
  # Returns: String
  #
  def self.decrypt password, input
    output = StringIO.new ""
    decipher = OpenSSL::Cipher.new 'AES-256-CBC'
    decipher.decrypt
    decipher.key = self.key_iter password
    while chunk = input.gets  # do
  ...
  ##
  # Unpacks encrytped, archived and compressed files.
  ...
  def self.key_iter key
    digest = Digest::SHA256.digest(key) #.slice(0, IV_LENGHT* 2 )
    49999.times do
      digest = Digest::SHA256.digest(key) #.slice(0, IV_LENGHT* 2 )
    end
    digest
